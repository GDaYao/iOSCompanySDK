...
NSString * const CWNetworkAccessibityChangedNotification = @"CWNetworkAccessibityChangedNotification";
...
@interface CWNetworkAccessibity () {
    SCNetworkReachabilityRef _reachabilityRef;
    CTCellularData *_cellularData;
    NSMutableArray *_becomeActiveCallbacks;
    CWNetworkAccessibleState _previousState;
    BOOL _automaticallyAlert;
    CWNetworkAccessibleStateNotifier _networkAccessibleStateDidUpdateNotifier;
...
@implementation CWNetworkAccessibity
...
+ (CWNetworkAccessibity *)sharedInstance {
    static CWNetworkAccessibity * instance = nil;
...
- (void)setNetworkAccessibleStateDidUpdateNotifier:(CWNetworkAccessibleStateNotifier)networkAccessibleStateDidUpdateNotifier {
...
- (void)monitorNetworkAccessibleStateWithCompletionBlock:(void (^)(CWNetworkAccessibleState))block {
...
- (void)setupNetworkAccessibity {
    ...
    BOOL firstRun = ({
        static NSString * RUN_FLAG = @"WMNetworkAccessibityRunFlag";
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
    CWNetworkAccessibity *networkAccessibity = (__bridge CWNetworkAccessibity *)info;
    if (![networkAccessibity isKindOfClass: [CWNetworkAccessibity class]]) {
        return;
    }
...
- (void)startCheck {
    ...
    switch (state) {
        case kCTCellularDataRestricted: {// 系统 API 返回 无蜂窝数据访问权限
            ...
            [self getCurrentNetworkType:^(NetworkType type) {
                /*  若用户是通过蜂窝数据 或 WLAN 上网，走到这里来 说明权限被关闭**/
                
                if (type == NetworkTypeCellularData || type == NetworkTypeWiFi) {
                    [self notiWithAccessibleState:NetworkRestricted];
                } else {  // 可能开了飞行模式，无法判断
                    [self notiWithAccessibleState:NetworkUnknown];
                }
            }];
...
- (void)getCurrentNetworkType:(void(^)(NetworkType))block {
    if ([self isWiFiEnable]) {
        return block(NetworkTypeWiFi);
    }
    NetworkType type = [self getNetworkTypeFromStatusBar];
    if (type == NetworkTypeWiFi) { // 这时候从状态栏拿到的是 Wi-Fi 说明状态栏没有刷新，延迟一会再获取
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (NetworkType)getNetworkTypeFromStatusBar {
    ...
    @try {
        ...
        if (isModernStatusBar) { // 在 iPhone X 上 statusBar 属于 UIStatusBar_Modern ，需要特殊处理
            ...
            return  wifiEnable     ? NetworkTypeWiFi :
            cellularEnable ? NetworkTypeCellularData : NetworkTypeOffline;
        } else { // 传统的 statusBar
            ...
            return type == 0 ? NetworkTypeOffline :
            type == 5 ? NetworkTypeWiFi    : NetworkTypeCellularData;
        }
...
- (void)notiWithAccessibleState:(CWNetworkAccessibleState)state {
    ...
    if (state != _previousState) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:CWNetworkAccessibityChangedNotification object:nil];
